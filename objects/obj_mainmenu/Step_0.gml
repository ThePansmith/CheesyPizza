...
switch state
{
	case states.titlescreen:
		...
		if ((keyboard_check_pressed(vk_anykey) || scr_checkanygamepad(obj_inputAssigner.player_input_device[0]) != -4 || scr_checkanystick(obj_inputAssigner.player_input_device[0])) && (!instance_exists(obj_mainmenu_jumpscare)))
		{
			...
			darkbuffer = 5
			jumpscarecount = 0
			sound_play_oneshot("event:/sfx/ui/lightswitch")
			with (obj_music)
	...
	case states.normal:
		...
		else
		{
			...
			if key_jump
			{
				...
				with (obj_menutv)
				{
					if (trigger == other.currentselect)
					{
						...
						sound_play_oneshot("event:/sfx/ui/fileselect")
						state = states.victory
			...
			else if (key_taunt2 && global.game_started[currentselect])
			{
				...
				deleteselect = 1
				sound_play_oneshot_3d("event:/sfx/voice/pig", 480, 270)
				switch currentselect
				{
					case 0:
						sprite_index = spr_titlepep_left
	...
	case states.bombdelete:
		...
		if ((deleteselect == 1 && key_jump) || (deleteselect == 0 && deletebuffer >= 120))
		{
			if (deleteselect == 0)
			{
				...
				sound_play_oneshot_3d("event:/sfx/misc/explosion", 480, 270)
				sound_play_oneshot_3d("event:/sfx/mort/mortdead", 480, 270)
				with (obj_menutv)
				{
