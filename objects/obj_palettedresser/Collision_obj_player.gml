...
if (other.key_up2)
{
	if (other.ispeppino) {
		with (other)
		{
			...
			{
				sound_play_oneshot_3d("event:/sfx/misc/clotheswitch", x, y);
		...
		var arr = palettes_noise[paletteselect];
		other.paletteselect = arr[2];
		if (arr[2] == 12)
			global.palettetexture = arr[3];
		else
		...
		ini_write_string("Game", "palettetexture", arr[0]);
		obj_savesystem.ini_str = ini_close();
		palettetitle = lang_get_value(concat("dresser_", palettes_noise[paletteselect][0], "title"));
		palettedesc = lang_get_value_newline(concat("dresser_", palettes_noise[paletteselect][0]));
	}
}
