...
if (!trapped)
{
	if (obj_player1.state == states.ratmountgrind || obj_player1.state == states.ratmountladder || obj_player1.state == states.ratmountcrouch || obj_player1.state == states.door)
	{
		wait = true;
	...
	if (comeback == 1)
	{
		...
		cbspeed = Approach(cbspeed, ptt ? max(20, abs(obj_player1.vsp * 2)) : 20, ptt ? 3 : 1);
	}
	else if (wait == 0)
	{
		x += hsp;
