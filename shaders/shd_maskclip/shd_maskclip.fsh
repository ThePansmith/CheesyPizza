...
uniform vec2 u_clip_sprite_uvs;
...
void main()
{
	...
	if (
		(v_vPosition.x >= u_clip_sprite_worldposition.x && v_vPosition.x <= (u_clip_sprite_worldposition.x + u_clip_sprite_size.x)) && 
		(v_vPosition.y >= u_clip_sprite_worldposition.y && v_vPosition.y <= (u_clip_sprite_worldposition.y + u_clip_sprite_size.y))
		)
	{
		vec2 position_in_clip = (v_vPosition - u_clip_sprite_worldposition) * u_clip_sprite_texelsize;

		vec4 clip_color = texture2D(u_clip_sprite_texture, position_in_clip + u_clip_sprite_uvs);
