...
function scr_pepperman_arenaintro()
{
	if (!skipintro)
	{
		...
		else if (sprite_index == spr_pepperman_intro2)
		{
			...
			image_index = 0;
			sound_play_oneshot_3d("event:/sfx/voice/peppermanlaugh", x, y);
			with (obj_player)
			{
...
function scr_pepperman_freefall()
{
	...
	if (grounded)
	{
		...
		else
		{
			...
			sound_play_oneshot_3d("event:/sfx/pep/groundpound", x, y);
			sound_play_oneshot_3d("event:/sfx/pepperman/groundpoundjump", x, y);
			if (x != targetplayer.x)
				image_xscale = sign(targetplayer.x - x);
...
function scr_pepperman_rolling()
{
	if (sprite_index != spr_pepperman_groundpoundland)
	{
		...
		if (check_solid(x + hsp, y) && !check_slope(x + hsp, y) && !place_meeting(x + hsp, y, obj_destructibles))
		{
			...
			image_xscale *= -1;
			vsp = -10;
			sound_play_oneshot_3d("event:/sfx/pepperman/groundpoundbump", x, y);
		}
		if (grounded && vsp > 0)
		{
			state = states.walk;
...
function scr_pepperman_shoulderbash()
{
	...
	if (_turn && !turned)
	{
		...
		if (ix != image_xscale)
		{
			sound_play_oneshot_3d("event:/sfx/pepperman/turn", x, y);
