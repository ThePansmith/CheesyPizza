		...
		if (_hurt)
		{
			...
			with (obj_bosscontroller)
			{
				if (!instance_exists(obj_hpeffect))
				{
					if (!global.boss_invincible)
					{
						...
						repeat (n)
						{
							...
							if (pos != undefined)
								scr_bosscontroller_particle_hp(spr_playerhp, irandom(sprite_get_number(spr_playerhp) - 1), pos[0], pos[1], 1, spr_peppalette, obj_player1.paletteselect, global.palettetexture);
				...
				else
				{
					...
					scr_bosscontroller_particle_hp(spr_playerhp, irandom(sprite_get_number(spr_playerhp) - 1), d.x, d.y, (d.x > (room_width / 2)) ? -1 : 1, spr_peppalette, obj_player1.paletteselect, global.palettetexture);
					instance_destroy(d);
				}
			}
		}
