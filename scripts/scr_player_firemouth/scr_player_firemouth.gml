function scr_player_firemouth()
{
	...
	if (sprite_index != spr_firemouthintro && sprite_index != spr_firemouthend)
	{
		...
		else if (sprite_index != spr_player_firemouthdash)
		{
			...
			if (!doublejump && input_buffer_slap > 0)
			{
				...
				if (move != 0)
					xscale = move;
				sound_play_oneshot_3d("event:/sfx/firemouth/dash", x + (12 * xscale), y);
				dir = xscale;
				movespeed = 12;
			}
			hsp = xscale * movespeed;
		...
		if (place_meeting(x + hsp, y, obj_solid) && !place_meeting(x + hsp, y, obj_slope_parent) && !place_meeting(x + hsp, y, obj_destructibles) && !place_meeting(x + hsp, y, obj_tntblock) && !place_meeting(x + hsp, y, obj_iceblock) && !place_meeting(x + hsp, y, obj_ratblock))
			movespeed = 0;
	...
	if (input_buffer_jump > 0 && can_jump && sprite_index != spr_firemouthintro && sprite_index != spr_firemouthend)
	{
		...
		input_buffer_jump = 0;
		sound_play_oneshot_3d("event:/sfx/firemouth/jump", x, y);
